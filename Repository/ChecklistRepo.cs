...
namespace AuditChecklistModule.Repository
{
    public class ChecklistRepo : IChecklistRepo
    {
        ...
        readonly log4net.ILog logs = log4net.LogManager.GetLogger(typeof(ChecklistRepo));
        ...
        public List<Questions> GetQuestions(string auditType)
        {
            try
            {
                logs.Info("Log from " + nameof(ChecklistRepo));
            ...
            {
                logs.Error("Exception " + e.Message + nameof(ChecklistRepo));
                return null;
            }
